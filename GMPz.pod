...
=head1 FUNCTIONS
   ...
   Rmpz_set_str       ($rop, $str, $base);
    ...
    If the string is deemed to be a non-valid base $base number, then
    the function croaks with an appropriate error message.
   ...
   $rop = Math::GMPz->new($arg [, $base]);
   $rop = Math::GMPz::new($arg [, $base]);
   $rop = new Math::GMPz($arg [, $base]);
   $rop = $op->new($arg [, $base]); # Added in Math-GMPz-0.69
    Returns a Math::GMPz object with the value of $arg.
    The optional argument $base is allowed only if $arg is a string.
    $arg can be either a Math::GMPz object, a Math::GMP object, a
    Math::GMPq object, a Math::MPFR object, a Math::BigInt object,
    a number (IV, UV or NV) or a string that represents an integer
    value. Strings that are written in scientific notation
    (eg '1e200' or '0x9p+40') are not currently accepted.  Such
    arguments could, eg, be made "acceptable" if   rewritten as
    '1'.('0' x 200) or, respectively, '0b1001'.('0'x40).
    If $arg is a string, the optional additional 2nd argument $base,
    specifying the base of the string, can be supplied to new(). If base
    is 0 (or not supplied) then the leading characters of the string are
    used:     0x or 0X for hex, 0b or 0B for binary, 0 for octal, or
    decimal otherwise. Legal values for the base are 0 and 2..62 .
    For any non-string argument that represents a non-integral value
    (ie NV, Math::GMPq object, or Math::MPFR object), the value is
    first rounded towards zero to an integral value, before being
    assigned to the Math::GMPz object.
   ...
   OPERATOR OVERLOADING
    ...
    Overloading works with numbers, strings (integer only), Math::GMPz
    objects, and Math::BigInt objects. Non-integer NV values will be
    truncated (ie rounded towards zero) to an integer value.

    I believe (not rigorously tested) that wherever the Math::GMPz
    operator overloading accepts a Math::GMPz object as the second arg,
    it will alternatively accept (and understand) a Math::GMP object as
    that second arg. Indications are that Math::GMP reciprocates.
    See also "INTEROPERABILITY WITH GMP", below.

    YMMV if you use old versions of Math::GMPq and Math::MPFR, but if you
    have Math-GMPq-0.61 (or later) and Math-MPFR-4.37 (or later) then, apart
    from the exceptions noted below, you will also be able to use Math::GMPq
    and Math::MPFR objects with the overloaded Math::GMPz operations. Such
    operations return Math::GMPq and (respectively) Math::MPFR objects.

   The following operators are overloaded:
    + - * / % **         # '**'  not usable with Math::GMPq objects.
                         # Returned Math::MPFR objects will have been
                         # calculated with default precision and rounding.

    += -= *= /= %= **=   # '**=' not usable with Math::GMPq objects.
                         # Returned Math::MPFR objects will have been
                         # calculated to the same precision as the given
                         # Math::MPFR operand, and with default rounding.

    << >> <<= >>=   # Not usable with Math::GMPq and Math::MPFR objects.
   ...
   INTEROPERABILITY WITH MATH::GMP

    In addition to Math::GMPz's overloaded operators accepting both
    Math::GMP and Math::GMPz objects (see "OPERATOR OVERLOADING"),
    Math::GMPz function calls seem to also accept Math::GMP and
    Math::GMPz objects interchangeably. (Not rigorously tested.)
...
=head1 LICENSE
    ...
    Copyright 2006-20011, 2013-26, Sisyphus
