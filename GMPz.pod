...
=head1 FUNCTIONS
   ...
   OPERATOR OVERLOADING
    ...
    objects, and Math::GMP objects. Non-integer NV values will be
    truncated (ie rounded towards zero) to an integer value.
    Overloading with Math::BigInt objects also works - but the extent of
    such support is not fully tested.

    If overloading Math::GMP objects, it is important that version 2.11 (or
    later) of Math::GMP is loaded, and that the Math::GMP_OLOAD module is
    also loaded. Arithmetic operations involving Math::GMPz and Math::GMP
    objects will then return a Math::GMPz object.
...
=head1 AUTHOR
    ...
    Sisyphus <sisyphus359 at(@) gmail dot (.) com>
