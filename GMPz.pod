...
=head1 FUNCTIONS
   ...
   OPERATOR OVERLOADING
    ...
    Math::BigInt objects, Math::GMPq objects and Math::MPFR objects.
   ...
   'Math::GMPz', 'Math::GMPq', or 'Math::MPFR' will not work.
    ...
    YMMV with earlier versions of Math::MPFR, but with Math-MPFR-4.35 and
    later, Math::MPFR objects can be provided as the second arg  with
    '+', '+=', '-', '-=', '*', '*=', '/', '/=, '%', '%=', '**' and '**=='
    operators - in which case the operation will return a Math::MPFR
    object (calculated using default Math::MPFR precision and default
    ...
    eg - evaluate ($mpfr_obj > $mpz_obj), not ($mpz_obj < $mpfr_obj).

    YMMV with earlier versions of Math::GMPq, but with Math-GMPq-0.61
    or later,  Math::GMPq objects can be provided as the second arg
    with the  comparison operators ( == != < <= > >= <=> ) and also
    with '+', '+=',  '-', '-=', '/', '/=', '%', '%=', '*' and '*='
