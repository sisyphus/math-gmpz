...
=head1 FUNCTIONS
   ...
   Rmpz_set_IV($rop, $iv); # $iv is a perl integer value (IV)
    ...
    IOK_flag($iv) will return a non-zero value if and only if
    the IOK flag is set.
   ...
   Rmpz_set_NV($rop, $nv); # $nv is a perl floating point value (NV)
    ...
    NOK_flag($nv) will return a non-zero value if and only if
    the NOK flag is set.
   ...
   $si = Rmpz_cmp_ui($op, $ui);
    ...
    NOTE:
         Rmpz_cmp_IV() and Rmpfr_cmp_NV() require that the 2nd
         argument has it's IOK/NOK flag set - otherwise the
         function croaks.
         IOK_flag($iv) and IOK_flag($nv) return a non-zero value
         if and only if the flag in question is set.
...
=head1 LICENSE
    ...
    Copyright 2006-20011, 2013-21, Sisyphus
