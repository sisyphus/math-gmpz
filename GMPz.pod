...
=head1 FUNCTIONS
   ...
   OPERATOR OVERLOADING
    ...
    TODO - check on the status of using Math::GMP objects with the
           overloaded operators, and fix any deficiencies.
    ...
    Math::MPFR with the overloaded Math::GMPz oprations.

   The following operators are overloaded:
    + - * / % **
    += -= *= /= %= **=
    << >> <<= >>=
    & | ^ ~   # Not yet usable with Math::GMPq and Math::MPFR objects
    &= |= ^=  # Not yet usable with Math::GMPq and Math::MPFR objects
    < <= > >= == != <=>
    ! abs
    =
    ...
    As their "exponent" argument, '**' and '**=' do not accept perl scalars
    that are either PVs (strings), NVs (floating point types) or negative
    IVs (integers). Values that are too large to fit into a C 'unsigned
    long int' will throw an error. (TODO - verify this claim.)
    TODO - allow PV exponents.
