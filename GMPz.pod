...
=head1 FUNCTIONS
   ...
   OPERATOR OVERLOADING
    ...
    YMMV if you use old versions of Math::GMPq and Math::MPFR, but if
    you have Math-GMPq-0.61 (or later) and Math-MPFR-4.37 (or later)
    then, apart from the exceptions noted below, you will be able to use
    Math::GMPq and Math::MPFR objects with the overloaded Math::GMPz
    operations. Such operations return Math::GMPq and (respectively)
    Math::MPFR objects. Returned Math::MPFR objects will have been
    calculated using current Math::MPFR default precision and current
    Math:::MPFR default rounding mode.
